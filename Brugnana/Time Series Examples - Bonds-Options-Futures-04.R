# In questo foglio andiamo ad osservare l'evoluzione dei tassi d'interese 
# riferiti ai mutui offerte dal dipartimento del tesoro Statunitense.
...
# Applications (with R Examples) 4th Edition
#
...
US_DTR_2020_df <- read.csv("data/Daily Treasury Rates - 2020.csv")
US_DTR_2021_df <- read.csv("data/Daily Treasury Rates - 2021.csv")
US_DTR_2022_df <- read.csv("data/Daily Treasury Rates - 2022.csv")
US_DTR_2023_df <- read.csv("data/Daily Treasury Rates - 2023.csv")

# Queste tabelle sono ordinate in ordine temporale decresente.
# Ordiniamole in modo crescente:
...
# Rimuoviamo i data_frame creati inizialmente e manteniamo solo il data_frame unione di tutti gli altri:
rm(US_DTR_2020_df)
rm(US_DTR_2021_df)
rm(US_DTR_2022_df)
rm(US_DTR_2023_df)
...
# removed.
save(US_DTR_2020_2023_df, file="saved_data/US_DTR_2020_2023_df.RData")

#
# Se hai bisogno di ricaricare i dati basta decommentare la riga sotto:
# load("saved_data/US_DTR_2020_2023_df.RData")

#------------------------------------------------------------------------------#
# Drawing plot -----------------------------------------------------------------
#
# To draw a plot of the Treasury Yield Curve Rates, 
# we need to manipulate the data frame *US_DTR_2020_2023_df*.
# First, we extract some rows (e.g., from March 31st to April 11th, 2023)
# from the data frame and rename the rows.
...
# Compare Plot 1 ---------------------------------------------------------------
# We compare the Treasury Yield Curve Rates in the period 03-31/04-11 2023 plotted above with the curve at the beginning of 2021, 2022, and 2023.
...
#-------------------------------------------------------------------------------
# Compute the Treasury ret. rates (Gennaio) ------------------------------------
# Now, we try to compute the Treasury ret. rates from the prices of Market Based Bills.
...
US_SP_2023_01_03_df <- read.csv("data/securityprice-2023-01-03.csv", header=FALSE)
...
# Compute the Treasury ret. rates (Aprile) -------------------------------------
# Still, we compute the Treasury par yield curve rates from the prices of Market Based Bills.
# https://www.treasurydirect.gov/GA-FI/FedInvest/selectSecurityPriceDate
# We build a data frame with data from the "data/securityprice-2023-04-11.csv" file.
US_SP_2023_04_11_df <- read.csv("data/securityprice-2023-04-11.csv", header=FALSE)
...
US_SP_BILLS_2023_04_11_Yield_Rate_lp <- ggplot(Data_df, aes(x=Index)) + 
  ...
  ylab("annual cont. comp. ret. (from Apr, 11th, 2023 to maturity date)") +
  guides(linetype="none", shape="none") +
  scale_colour_manual(name="Legend", labels=leg_line_labs, values=leg_line_cols, breaks=leg_line_breaks) +
...
# from March, 31st to April 11th 2023. 
# Then, we build a data frame with data from the "TotalOutput_2023-03-31_2023-04-11.csv" file.
US_SP_2023_03_31_2023_04_11_df <- read.csv("data/TotalOutput_2023-03-31_2023-04-11.csv", header=TRUE)
...
# Consider the future on S&P 500 E-Mini Jun 23 (ES=F) or (ESM24.CME)
...
tail(ESM24.CME_df)
# We add a Date column to the data frame *ESM24.CME_df*.
ESM24.CME_df <- add_column(.data=ESM24.CME_df, Date=as.Date(rownames(ESM24.CME_df), format="%Y-%m-%d"), 
                           .before="ESM24.CME.Open")
ESM24.CME_df
head(ESM24.CME_df)
tail(ESM24.CME_df)
...
y_breaks_num <- 10
y_max <- max(na.rm(Data_df$GSPC.Adjusted),na.rm(Data_df$ESM24.CME.Adjusted))
y_min <- min(na.rm(Data_df$GSPC.Adjusted),na.rm(Data_df$ESM24.CME.Adjusted))
y_binwidth <- round((y_max-y_min)/y_breaks_num, digits=3)
y_breaks_low <- floor(y_min/y_binwidth)*y_binwidth
y_breaks_up <- ceiling(y_max/y_binwidth)*y_binwidth
y_breaks <- round(seq(from=y_breaks_low, to=y_breaks_up, by=y_binwidth),3)
...
SP_500_EMini_Adj_Prices_lp <- ggplot(Data_df, aes(x=Index)) + 
  ...
  geom_line(aes(y=ESM24.CME.Adjusted, color="line_col_r"), linewidth=0.8, linetype="solid") +
  scale_x_continuous(breaks=x_breaks, labels=x_labs, limits=x_lims) +
  scale_y_continuous(breaks=y_breaks, labels=NULL, limits=y_lims,
                     sec.axis= sec_axis(~., breaks=y_breaks, labels=y_labs)) +
  ggtitle(title_content) +
...
J <- 0.5
x_lims <- c(x_breaks_low-J*x_binwidth, x_breaks_up+J*x_binwidth)
y_name <- bquote("annual cont. comp. ret. (from Jan, 3rd, 2023 to maturity date)")
y_breaks_num <- 10
y_max <- max(na.rm(as.numeric(sub("%","", Data_df$Perc.Ret.Rate.at.Maturity))))
y_min <- min(na.rm(as.numeric(sub("%","", Data_df$Perc.Ret.Rate.at.Maturity))))
y_binwidth <- round((y_max-y_min)/y_breaks_num, digits=3)
y_breaks_low <- floor(y_min/y_binwidth)*y_binwidth
y_breaks_up <- ceiling(y_max/y_binwidth)*y_binwidth
y_breaks <- round(seq(from=y_breaks_low, to=y_breaks_up, by=y_binwidth),3)
...
K <- 0
y_lims <- c((y_breaks_low-K*y_binwidth), (y_breaks_up+K*y_binwidth))
line_col_b <- bquote("annual cont. comp. ret. ")
leg_line_labs   <- c(line_col_b)
leg_line_cols   <- c("line_col_b"="blue")
leg_line_breaks <- c("line_col_b")
Risk_Free_Rate_06_16_2023_lp <- ggplot(Data_df, aes(x=Index)) + 
  geom_line(aes(y=as.numeric(sub("%","", Perc.Ret.Rate.at.Maturity)), color="line_col_b"), linewidth=0.8, linetype="solid") +
