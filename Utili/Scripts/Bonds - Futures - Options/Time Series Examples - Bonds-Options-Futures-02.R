...
length <- (ncol(selec_rows_US_DTR_2020_2023_df)-1)
...
leg_labs <- as.character(US_DTR_2020_2023_df$Date[selec_rows])
...
ref_dates <- unique(Data_df$Date)
length(ref_dates)
mat_dates_subsets <- list()
for(j in 1:length(ref_dates)){
  mat_dates_subsets[[j]] <- Data_df$Maturity.Date[which(Data_df$Date==ref_dates[j])]
}
show(mat_dates_subsets)
mat_dates_cup <- mat_dates_subsets[[1]]
for(j in 2:length(ref_dates)){
...
for(j in 1:length(ref_dates)){
  US_SP_BILLS_2023_03_31_2023_04_11_ls[[j]] <- subset(Data_df, Data_df$Date==ref_dates[j])
  ...
  for(k in 1:length(mat_dates_cup)){
    ...
    else{US_SP_BILLS_2023_03_31_2023_04_11_ls[[j]] <- arrange(rbind(US_SP_BILLS_2023_03_31_2023_04_11_ls[[j]], c(rep(NA,(mat_date_pos-1)),mat_dates_cup[k],rep(NA,(ncol(Data_df)-mat_date_pos)-1),ref_dates[j])), Maturity.Date)}
  }
}
#
for(j in 1:length(ref_dates)){
...
#
Data_df <- US_SP_BILLS_2023_03_31_2023_04_11_df
First_Day <- as.character(first(ref_dates))
Last_Day <- as.character(last(ref_dates))
max_path_length <- length(mat_dates_cup)
min_path_length <- length(mat_dates_cap)
title_content <- bquote(atop("University of Roma \"Tor Vergata\" - \u0040 MPSMF 2022-2023", 
...
path_max <- c()
for(j in 1:length(ref_dates)){
  path_max[j] <- max(na.rm(as.numeric(sub("%","", subset(Data_df, Data_df$Date == ref_dates[j])$Perc.Ann.Rate.of.Ret))))
}
y_max <- max(path_max)
path_min <- c()
for(j in 1:length(ref_dates)){
  path_min[j] <- min(na.rm(as.numeric(sub("%","", subset(Data_df, Data_df$Date == ref_dates[j])$Perc.Ann.Rate.of.Ret))))
}
y_min <- min(path_min)
y_binwidth <- round((y_max-y_min)/y_breaks_num, digits=3)
y_breaks_low <- floor(y_min/y_binwidth)*y_binwidth
...
K <- 0
y_lims <- c((y_breaks_low-K*y_binwidth), (y_breaks_up+K*y_binwidth))
line_cols <- rainbow(length(ref_dates))
leg_line_1 <- as.character(ref_dates)[1]
leg_line_2 <- as.character(ref_dates)[2]
leg_line_3 <- as.character(ref_dates)[3]
leg_line_4 <- as.character(ref_dates)[4]
leg_line_5 <- as.character(ref_dates)[5]
leg_line_6 <- as.character(ref_dates)[6]
leg_line_7 <- as.character(ref_dates)[7]
leg_line_8 <- as.character(ref_dates)[8]
...
leg_line_breaks <- c("leg_line_1", "leg_line_2", "leg_line_3", "leg_line_4", "leg_line_5", "leg_line_6", "leg_line_7",
                     "leg_line_8")
# leg_line_labs_bis <- as.character(ref_dates)
...
US_SP_BILLS_2023_03_31_2023_04_11_lp <- ggplot(Data_df) +
   geom_line(data=subset(Data_df, Data_df$Date == ref_dates[1]), alpha=1, linewidth=0.8, linetype="solid", 
              aes(x=Loc_Index, y=as.numeric(sub("%","", Perc.Ann.Rate.of.Ret)), color="leg_line_1", group=1)) +
   geom_line(data=subset(Data_df, Data_df$Date == ref_dates[2]), alpha=1, linewidth=0.8, linetype="solid",
             aes(x=Loc_Index, y=as.numeric(sub("%","", Perc.Ann.Rate.of.Ret)), color="leg_line_2", group=1)) +
   geom_line(data=subset(Data_df, Data_df$Date == ref_dates[3]), alpha=1, linewidth=0.8, linetype="solid",
             aes(x=Loc_Index, y=as.numeric(sub("%","", Perc.Ann.Rate.of.Ret)), color="leg_line_3", group=1)) +
   geom_line(data=subset(Data_df, Data_df$Date == ref_dates[4]), alpha=1, linewidth=0.8, linetype="solid",
             aes(x=Loc_Index, y=as.numeric(sub("%","", Perc.Ann.Rate.of.Ret)), color="leg_line_4", group=1)) +
   geom_line(data=subset(Data_df, Data_df$Date == ref_dates[5]), alpha=1, linewidth=0.8, linetype="solid",
             aes(x=Loc_Index, y=as.numeric(sub("%","", Perc.Ann.Rate.of.Ret)), color="leg_line_5", group=1)) +
   geom_line(data=subset(Data_df, Data_df$Date == ref_dates[6]), alpha=1, linewidth=0.8, linetype="solid",
             aes(x=Loc_Index, y=as.numeric(sub("%","", Perc.Ann.Rate.of.Ret)), color="leg_line_6", group=1)) +
   geom_line(data=subset(Data_df, Data_df$Date == ref_dates[7]), alpha=1, linewidth=0.8, linetype="solid",
             aes(x=Loc_Index, y=as.numeric(sub("%","", Perc.Ann.Rate.of.Ret)), color="leg_line_7",  group=1)) +
   geom_line(data=subset(Data_df, Data_df$Date == ref_dates[8]), alpha=1, linewidth=0.8, linetype="solid",
             aes(x=Loc_Index, y=as.numeric(sub("%","", Perc.Ann.Rate.of.Ret)), color="leg_line_8",  group=1)) +
#   annotate(geom = "text", size=3.0, x=x_breaks, y=y_breaks_low, label=x_labs, vjust=-0.5)
#   annotate(geom = "text", size=3.0, x=x_breaks, y=y_breaks_low, label=x_labs, vjust=-0.5) +
   scale_x_continuous(breaks=x_breaks, labels=x_labs, limits=x_lims) +
   scale_y_continuous(breaks=y_breaks, labels=NULL, limits=y_lims,
                      sec.axis= sec_axis(~., breaks=y_breaks, labels=y_labs)) +
...
# y_lims <- c((y_breaks_low-K*y_binwidth), (y_breaks_up+K*y_binwidth))
# 
# leg_vals <- rainbow(length(levels(factor(Data_df$Loc_Index))))[as.numeric(levels(factor(Data_df$Loc_Index)))]
# US_SP_BILLS_2023_03_31_2023_04_11_bis_lp <- ggplot(Data_df, aes(x=Maturity.Date, y=Perc.Ann.Rate.of.Ret, group=factor(Loc_Index))) + 
#   geom_line(aes(color=factor(Loc_Index)), linewidth=0.8, linetype="solid") +
#   ggtitle(title_content) +
#   labs(subtitle=subtitle_content, caption=caption_content) +
#   xlab("time to maturity") + ylab("yield rates") +
#   scale_colour_manual(name="Legend", labels=leg_line_labs, values=leg_line_cols, breaks=leg_line_breaks) +
...
#         axis.text.x = element_text(angle=0, vjust=1),
#         legend.key.width = unit(0.80,"cm"), legend.position="right")
# plot(US_SP_BILLS_2023_03_31_2023_04_11_bis_lp)
...
ESM23.CME_df <- add_column(.data=ESM23.CME_df, GSPC.Adjusted=GSPC_df$GSPC.Adjusted, .after="ESM23.CME.Adjusted" )
head(ESM23.CME_df)
...
ESM23.CME_df <- add_column(.data=ESM23.CME_df, 
                            Days.to.Maturity=as.vector(difftime(as.Date(as.character("2023-06-16")), ESM23.CME_df$Date, units="days")), 
                            .after="GSPC.Adjusted")
head(ESM23.CME_df)
tail(ESM23.CME_df)
# We also add the column *Months.to.Maturity* [resp. *Years.to.Maturity*], which accounts for the number of months 
...
ESM23.CME_df <- add_column(.data=ESM23.CME_df, 
                            Months.to.Maturity=as.vector(ESM23.CME_df$Days.to.Maturity/30.4369),
                            Years.to.Maturity=as.vector(ESM23.CME_df$Days.to.Maturity/365.2425),
                            .after="Days.to.Maturity")
head(ESM23.CME_df)
tail(ESM23.CME_df)
#
...
#
ESM23.CME_df <- add_column(.data=ESM23.CME_df, 
                           Rate.of.Ret.at.Maturity=(ESM23.CME_df$ESM23.CME.Adjusted/ESM23.CME_df$GSPC.Adjusted-1),
                           Perc.Rate.of.Ret.at.Maturity=label_percent(accuracy = 0.001)(ESM23.CME_df$ESM23.CME.Adjusted/ESM23.CME_df$GSPC.Adjusted-1),
                           Ann.Rate.of.Ret.at.Maturity=((ESM23.CME_df$ESM23.CME.Adjusted/ESM23.CME_df$GSPC.Adjusted)^(365.2425/ESM23.CME_df$Days.to.Maturity)-1),
                           Perc.Ann.Rate.of.Ret.at.Maturity=label_percent(accuracy = 0.001)((ESM23.CME_df$ESM23.CME.Adjusted/ESM23.CME_df$GSPC.Adjusted)^(365.2425/ESM23.CME_df$Days.to.Maturity)-1),
...
link_ESM23 <- "https://finance.yahoo.com/quote/ES%3DF?p=ES%3DF"
caption_content <- "Author: Roberto Monte"
subtitle_content <- bquote(paste("path length ", .(DS_length), " sample points. Data by courtesy of Yahoo Finance -  ", .(link_GSPC),~~"-"~~.(link_ESM23)))
x_name <- bquote()
# To obtain the sub-multiples of the length of the data set as a hint on the number of breaks to use
# library(numbers)
# primeFactors(DS_length)
...
Risk_Free_Rate_06_16_2023_lp <- ggplot(Data_df, aes(x=Index)) + 
  ...
  annotate(geom = "text", size=3.0, x=x_breaks, y=y_breaks_low, label=x_labs_1, vjust=0.5)
...
#
Call_LastTrDate_df <- data.frame(Call_LastTrDate=as.Date(SPX_Opt_2023_04_11_06_16_df$Call_LastTrTime, format="%d/%m/%Y"))
...
#
Put_LastTrDate_df <- data.frame(Put_LastTrDate=as.Date(SPX_Opt_2023_04_11_06_16_df$Put_LastTrTime, format="%d/%m/%Y"))
...
title_content <- bquote(atop("University of Roma \"Tor Vergata\" - \u0040 MPSMF 2022-2023", 
                             paste("Scatter Plot of the Call-Put Difference Against the Strike Price")))
subtitle_content <- bquote(paste("Data set size",~~.(n),~~"sample points;    Evaluation Date 2023-05-12;   Maturity Date 2023-06-16"))
caption_content <- "Author: Roberto Monte" 
# To obtain the submultiples of the length of the data set as a hint on the number of breaks to use
# library(numbers)
# primeFactors(n)
...
# 4.828% 
# computed corresponding to a maturity of 65 days in the data frame *US_SP_BILLS_2023_04_11_df*
#

SPX_Opt_2023_04_11_06_16_df <- read.csv("SPX_Option_Chain_2023_04_11_06_16.csv")
head(SPX_Opt_2023_04_11_06_16_df)
#
Call_LastTrDate_df <- data.frame(Call_LastTrDate=as.Date(SPX_Opt_2023_04_11_06_16_df$Call_LastTrTime))
class(Call_LastTrDate_df)
head(Call_LastTrDate_df,20)
nrow(Call_LastTrDate_df)
Call_LastTrDate_tb <- table(Call_LastTrDate_df)   
